...
const OurPartnerCard = ({ partner }) => (
    <Card className="partner_card">
        <img className='partnerCard_img' src= {partner.Developer_Logo ? `${BASE_URL}/uploads/${partner.Developer_Logo}` : '/img/placeholder-logo.png'} alt='logo' />
    </Card>
);
...
const OurPartner = () => {
    ...
    useEffect(() => {
        fetch(`${BASE_URL}/properties`) // Backend API endpoint
            .then(res => res.json())
